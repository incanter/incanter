...
@java -Dincanter.home=/path/to/incanter/dir -jar modules/incanter-bundle/target/incanter-exec.jar@
...
or run: @mvn clojure:repl@ , from the @modules/incanter-bundle@ directory

A Swank server can be started with the @bin/swank@ script or by running @mvn clojure:swank@, from the @modules/incanter-bundle@ directory, which can be connected to with @M-x slime-connect@ within Emacs.
