...
(deftest set-fill-between-test
  (let [hw1 (view (doto (xy-plot (range 5)
                                 (range 5))
                    (add-lines (range 5)
                               (repeat 5 1))
                    (set-fill-between)))
        hw2 (view (doto (xy-plot (range 5)
                                 (range 5))
                    (add-lines (range 5)
                               (repeat 5 1))
                    (set-fill-between)
                    (add-lines (range 10 15)
                               (range 5))
                    (add-lines (range 10 15)
                               (repeat 5 1))
                    (set-fill-between :positive-paint (java.awt.Color/GREEN)
